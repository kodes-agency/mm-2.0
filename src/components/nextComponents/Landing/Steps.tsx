...
export const Steps = ({ block }: { block: Steps }) => {
  return (
    <section
      className={`${getStyle(block.style.style)} flex flex-col items-center justify-center py-20 px-6 md:px-10 space-y-20`}
    >
        ...
        <div className='relative flex items-center justify-center max-w-4xl'>
            <div className="grid grid-cols-1 gap-10 md:gap-0">
                {block.content.steps.map((step, index) => (
                    <div key={index} className={`flex flex-col space-y-2 pl-5 md:w-1/2 ${index % 2 === 0 ? "md:items-end pr-10" : "md:items-start md:translate-x-full md:pl-10"}`}>
                        <h3 className={` text-6xl font-bold ${index % 2 === 0 ? "md:text-end" : "md:text-start"}`}>{index+1}.</h3>
                        <h4 className={` text-2xl font-bold ${index % 2 === 0 ? "md:text-end" : "md:text-start"}`}>{step.title}</h4>
                        <p className={` ${index % 2 === 0 ? "md:text-end" : "md:text-start"}`}>{step.description}</p>
                    </div>
                ))}
            </div>
