...
export const Statistics = ({ block }: { block: Statistics }) => {
  return (
    <section
      className={`${getStyle(block.style.style)} flex flex-col items-center justify-center py-20 px-6 md:px-10 space-y-10`}
    >
      ...
      {block.content.hasButton === 'yes' && (
        <Button text={block.content.buttonText || ''} link={block.content.buttonUrl || ''}></Button>
      )}
    </section>
  )
}
